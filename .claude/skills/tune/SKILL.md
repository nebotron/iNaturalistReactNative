...
- Eval: `python3 scripts/eval_onnx_variants.py crop_training.json` — **not** `evaluate_subject_detector.py` (uses COCO PyTorch weights, misleadingly low ~0.72)
...
Trained on 402 local images (not Firebase) → regressed Firebase score to 0.689. Reverted. Firebase data saved as canonical `crop_training.json`. Retraining on Firebase-only data pending.

**Key lessons:**
- Always eval on Firebase (`crop_training.json`) — local images are a different distribution
- After retraining, sweep algo (top1/union) and padding — optimal changes per model
- `set_classes()` is YOLO World-only; crashes on standard YOLOv8n
