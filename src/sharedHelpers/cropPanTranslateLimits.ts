...
export function computeCropPanTranslateLimits(
  ...
  // keeps it fully inside on that axis -- panning is left unconstrained
  // there rather than being pinned to a single forced position. The other
  ...
  let minTotalTranslateX = -Infinity;
  let maxTotalTranslateX = Infinity;
  if ( cropFractionW <= 1 ) {
  ...
  let minTotalTranslateY = -Infinity;
  let maxTotalTranslateY = Infinity;
  if ( cropFractionH <= 1 ) {
