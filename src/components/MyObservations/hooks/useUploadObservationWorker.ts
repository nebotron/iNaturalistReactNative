...
import { MS_BEFORE_TOOLBAR_RESET, MS_BEFORE_UPLOAD_TIMES_OUT } from "./useUploadObservations";
...
const useUploadObservationWorker = ( ) => {
  ...
  const uploadObservationAndCatchError = useCallback( async ( observation: RealmObservation ) => {
    ...
    try {
      ...
      // A batch the user stopped is not a batch that failed, so don't leave
      // the toolbar reporting errors for uploads they cancelled themselves.
      ...
      // stopAllUploads already put the session in its cancelled state; calling
      // completeUploads here would relabel a batch the user stopped as one
      // that finished.
      if ( !abortController?.signal.aborted
        && freshState.uploadQueue.length === 0
