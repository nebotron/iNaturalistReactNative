...
import { Pressable, fontMonoClass, View } from "components/styledComponents";
      ...
      <View className="flex-row items-center flex-wrap gap-x-2">
        ...
        {entry.status > 0 && (
          <Text
            className={classnames(
              fontMonoClass,
              "text-xs",
              is4xx5xx || isError ? "text-warningRed" : "text-darkGray",
            )}
          >
            {entry.status}
          </Text>
        ...
        {entry.cacheHit !== null && (
          <View
            className={classnames(
              "px-1 rounded-sm",
              entry.cacheHit ? "bg-inatGreen" : "bg-mediumGray",
            )}
          >
