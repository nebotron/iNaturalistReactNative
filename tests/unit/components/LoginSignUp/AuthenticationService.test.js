...
describe( "getJWT 401 handling", ( ) => {
  ...
  describe( "with a stored refresh token", ( ) => {
    beforeEach( ( ) => {
      const service = RNSInfo.stores.get( "app" );
      service.set( "refreshToken", "test-refresh-token" );
    } );

    it( "refreshes the access token and retries instead of navigating on 401", async ( ) => {
      nock( API_HOST )
        .get( "/users/api_token.json" )
        .reply( 401 )
        .post( "/oauth/token" )
        ...
        .get( "/users/api_token.json" )
        .reply( 200, { api_token: "fresh-jwt" } );

      const result = await getJWT( );

      expect( result ).toEqual( "fresh-jwt" );
      expect( navigationRef.navigate ).not.toHaveBeenCalled( );
    } );

    it( "persists the rotated access and refresh tokens after a refresh", async ( ) => {
      nock( API_HOST )
        .get( "/users/api_token.json" )
        .reply( 401 )
        .post( "/oauth/token" )
      ...
      const service = RNSInfo.stores.get( "app" );
      expect( service.get( "accessToken" ) ).toEqual( "new-access-token" );
      expect( service.get( "refreshToken" ) ).toEqual( "new-refresh-token" );
    } );

    it( "navigates to login when the refresh also fails", async ( ) => {
      nock( API_HOST )
        .get( "/users/api_token.json" )
        .reply( 401 )
        .post( "/oauth/token" )
