...
git -C "$REPO_ROOT" rev-parse "origin/$BRANCH" 2>/dev/null > "$SEEN_HASH_FILE" || echo "" > "$SEEN_HASH_FILE"
...
while true; do
  ...
  git -C "$REPO_ROOT" fetch origin "$BRANCH" --quiet 2>/dev/null || {
    echo "$(date): fetch failed, retrying next cycle" >&2
    continue
  }

  NEW_HASH="$(git -C "$REPO_ROOT" rev-parse "origin/$BRANCH" 2>/dev/null || true)"
