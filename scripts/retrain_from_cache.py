...
def main() -> None:
    ...
    parser.add_argument("--no-install", action="store_true",
                        help="Skip installing the exported ONNX over the deployed model")
    parser.add_argument("--heldout-urls", default=None,
    ...
    for entry in entries:
        ...
        resolved.append((img_path, cx, cy, w, h, url))
    ...
    for split_name, items in [("train", train_entries), ("val", val_entries)]:
        ...
        for i, (img_path, cx, cy, w, h, url) in enumerate(items):
            ...
            manifest[f"{split_name}/{i:05d}.jpg"] = url
    ...
    if args.no_install:
        print(f"--no-install: leaving deployed model untouched (export at {export_path})")
        return
